...
* **The Principle**:
  ...
  * **Credential and Environment Constraints**: If a task requires new external API keys, credentials, or environment secrets, the agent **must not** mock them in a way that risks production failure. Instead, it must list the required keys in the `implementation_plan.md` under "Open Questions" or "User Review Required", set `RequestFeedback: true`, and wait for the user to securely set up the keys in the environment or GitHub Secrets and approve the plan.
  * **Telemetry & Self-Healing**: Expose structured application error logs and performance metrics to the agent CLI. This enables incoming agents to autonomously review production diagnostic data, prioritize fixes in the playbook backlog, and self-heal regressions.
  * **Growth vs. Feature Balance**: The developer agent must not focus solely on technical or architectural features. It must maintain a healthy balance by regularly introducing and executing playbook tasks dedicated to growth, including SEO/GEO optimizations, viral sharing hooks, indexation improvements, and user traction tactics to continuously drive traffic.
