...
*   **`[AC-2] Intent Test Clipboard Permissions`**:
    All E2E tests verifying outbound sharing intents (clicking `#btn-post-share`) must grant clipboard permissions (`await context.grantPermissions(['clipboard-read', 'clipboard-write']);`) before navigating to prevent clipboard write permission failures in headless browsers.
*   **`[AC-3] Share Button Loading Guards`**:
    The "Post Now" (`#btn-post-share`) and "Copy" (`#btn-copy-share`) buttons must be disabled and styled with reduced opacity and a `not-allowed` cursor when the preview textarea contains `"Generating post..."`, `"Error generating post. Please try again."`, or is completely empty. They must be re-enabled only when post generation completes successfully.
*   **`[AC-4] Automated Copy on Share`**:
    Clicking `#btn-post-share` must automatically write the preview text to the clipboard using `navigator.clipboard.writeText`. If the clipboard API throws an error (e.g., due to restricted environment permissions), the error must be caught gracefully and the outbound intent link redirect must still proceed.
...
*   **`public/app.js`**:
    *   Update `updatePreviewAndValidation()` to check if text is generating (`"Generating post..."`), empty, or has errored, and conditionally apply the `disabled` property and `.disabled` class to `#btn-post-share` and `#btn-copy-share`.
    *   Modify the `#btn-post-share` click handler to first attempt `navigator.clipboard.writeText()` and display `#share-toast`, and then trigger `window.open` for the platform sharing URL.
...
*   **`tests/pinterest-sharing-suite.spec.js`**:
    *   Update tests verifying sharing to wait for `.trend-item.active` and grant clipboard permissions.
...
*   **Objection**: Will adding clipboard write to the share button break headless browser runs on standard intent tests?
    *   *Resolution*: Yes, unless permissions are granted. This spec explicitly mandates that the E2E tests for these actions must call `context.grantPermissions` before clicking `#btn-post-share`, and that the client-side code will run clipboard operations inside a `try/catch` block to prevent failure-to-copy from blocking the sharing redirect.
...
*   **Independent**: Yes
*   **Description**: Add safety guards to disable the share/copy buttons when text is empty, generating, or errored. Inject the `#share-toast` notification structure and style sheet. Implement the copy-to-clipboard hook and toast activation on clicking "Post Now".
...
*   **Description**: Update test cases to wait for `.trend-item.active` to eliminate page-load race conditions. Add clipboard permissions to all tests clicking `#btn-post-share`.
