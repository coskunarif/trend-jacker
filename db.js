  ...
  try {
    ...
    sqliteDb.exec(`
      CREATE TABLE IF NOT EXISTS debate_votes (
        trend TEXT PRIMARY KEY,
        optimist_wins INTEGER DEFAULT 0,
        skeptic_wins INTEGER DEFAULT 0
      )
    `);
...
/**
 * Retrieves the debate verdict/wins data for a given trend.
 * @param {string} trend 
 * @returns {Promise<{optimistWins: number, skepticWins: number}>}
 */
export async function getDebateData(trend) {
  if (firestore) {
    try {
      const docRef = firestore.collection('debates').doc(trend);
      const doc = await docRef.get();
      if (doc.exists) {
        const data = doc.data();
        return {
          optimistWins: data.optimistWins || 0,
          skepticWins: data.skepticWins || 0
        };
      }
      return { optimistWins: 0, skepticWins: 0 };
    } catch (err) {
      console.error(`Firestore error in getDebateData for "${trend}":`, err.message);
    }
  }
  ...
  if (sqliteDb) {
    try {
      const stmt = sqliteDb.prepare('SELECT optimist_wins, skeptic_wins FROM debate_votes WHERE trend = ?');
      const row = stmt.get(trend);
      if (row) {
        return { optimistWins: row.optimist_wins, skepticWins: row.skeptic_wins };
      }
      return { optimistWins: 0, skepticWins: 0 };
    } catch (err) {
      console.error(`Local SQLite query failed for debate "${trend}":`, err.message);
    }
  }

  // In-memory fallback
  if (!inMemoryStorage.has(`debate-${trend}`)) {
    inMemoryStorage.set(`debate-${trend}`, { optimistWins: 0, skepticWins: 0 });
  }
  return inMemoryStorage.get(`debate-${trend}`);
}

/**
 * Increments the debate win count for a trend (either 'optimist' or 'skeptic') and returns the updated counts.
 * @param {string} trend 
 * @param {'optimist'|'skeptic'} winner 
 * @returns {Promise<{optimistWins: number, skepticWins: number}>}
 */
export async function incrementDebateVote(trend, winner) {
  const field = winner === 'optimist' ? 'optimistWins' : 'skepticWins';
  if (firestore) {
    try {
      const docRef = firestore.collection('debates').doc(trend);
      await docRef.set({
      ...
      console.error(`Firestore error in incrementDebateVote for "${trend}":`, err.message);
    }
  }

  // Get current state to return after SQLite/in-memory update
  const current = sqliteDb
    ? await getLocalSqliteDebateData(trend)
    : (inMemoryStorage.get(`debate-${trend}`) || { optimistWins: 0, skepticWins: 0 });
  
  if (winner === 'optimist') {
    current.optimistWins++;
  } else {
    current.skepticWins++;
  }
  ...
  if (sqliteDb) {
    try {
      sqliteDb.prepare('INSERT OR IGNORE INTO debate_votes (trend, optimist_wins, skeptic_wins) VALUES (?, 0, 0)').run(trend);
      if (winner === 'optimist') {
        sqliteDb.prepare('UPDATE debate_votes SET optimist_wins = optimist_wins + 1 WHERE trend = ?').run(trend);
      } else {
        sqliteDb.prepare('UPDATE debate_votes SET skeptic_wins = skeptic_wins + 1 WHERE trend = ?').run(trend);
      }
      return current;
    } catch (err) {
      console.error(`Local SQLite write failed for debate "${trend}":`, err.message);
    }
  }

  // In-memory fallback update
  inMemoryStorage.set(`debate-${trend}`, current);
  return current;
}

async function getLocalSqliteDebateData(trend) {
  try {
    const stmt = sqliteDb.prepare('SELECT optimist_wins, skeptic_wins FROM debate_votes WHERE trend = ?');
    const row = stmt.get(trend);
    if (row) {
